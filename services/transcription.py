...
def process_transcription(media_url, output_type, max_chars=56, language=None,):
    ...
    try:
        ...
        if output_type == 'transcript':
            segments_gen, info = model.transcribe(input_filename, language=language)
            # Collect all text from segments
            output = ''.join([seg.text for seg in segments_gen])
            logger.info("Generated transcript output")
        elif output_type in ['srt', 'vtt']:
            segments_gen, info = model.transcribe(input_filename)
            srt_subtitles = []
