...
def generate_transcription(video_path, language='auto'):
    try:
        ...
        segments_gen, info = model.transcribe(video_path, **transcription_options)
        
        # Convert generator to list with proper structure
        segments_list = []
...
def determine_alignment_code(position_str, alignment_str, x, y, video_width, video_height):
    ...
    # If x and y are provided, use them directly and set \an based on alignment_str OR position_str if available
    if x is not None and y is not None:
        logger.info("[determine_alignment_code] x and y provided. Using position to determine anchor.")
        
        # Check if position_str maps to a valid alignment code
        if position_str in POSITION_ALIGNMENT_MAP:
            an_code = POSITION_ALIGNMENT_MAP[position_str]
        else:
            # Fallback to previous logic: Middle row + alignment
